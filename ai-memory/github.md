...
- 「PR作って」は、PR作成だけでなくmergeとローカル掃除まで含む完了フローとして扱う。
- 完了フローでは、作業ブランチ作成、変更確認、検証、commit、push、PR作成、コードレビュー、CI確認を行う。問題がなければsquash自動マージを設定し、merge完了まで確認する。
- merge後は既定branch（`main`または`master`）へ切り替え、対応するremote branchをpullし、merge済みのローカル作業ブランチを削除して、`git fetch --prune`を実行する。
- 最後に`git branch -a`と`git status --short --branch`を確認し、不要なローカルbranchや未追跡ファイルが残っていれば内容を確認して整理する。
